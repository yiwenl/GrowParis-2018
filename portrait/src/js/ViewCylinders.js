...
class ViewCylinders extends alfrid.View {
	...
	render(mShadowMatrix, mStep) {
		...
		if(mStep === 0) {
			this.shader.uniform("uUseColor", "float", 1);
			this.shader.uniform("uScaleOffset", "float", 0);
		} else if(mStep === 1) {
			this.shader.uniform("uUseColor", "float", 0);
			this.shader.uniform("uScaleOffset", "float", 1);
		} else if(mStep === 2) {
			this.shader.uniform("uUseColor", "float", 1);
			this.shader.uniform("uScaleOffset", "float", 1);
		} else if(mStep === 3) {
